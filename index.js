...
let shaderProgram = compileProgram(fs.readFileSync(__dirname + "/shaders/shader-vs.glsl", { encoding: 'utf8' }), fs.readFileSync(__dirname + "/shaders/shader-fs.glsl", { encoding: 'utf8' }));
...
gl_matrix_1.mat4.perspective(pMatrix, 45, options.width / options.height, 0.1, 20.0);
let vertices = [];
let colors = [];
var Axis;
(function (Axis) {
...
class Cube {
    constructor(timeStart, timeEnd, axis) {
        ...
        this.color.push(Math.random() * .4 + .3, Math.random() * .4 + .3, Math.random() * .4 + .3);
    }
    render(time, alpha, x, y, z, dx, dy, dz) {
        ...
        if (!this.left && Math.random() < (1 - fraction) * (1 - fraction) / 20)
            this.left = new Cube(time, time + 5000, (this.axis + 1 + Math.floor(Math.random() * 2)) % 3);
        if (!this.right && Math.random() < fraction * fraction / 20)
            this.right = new Cube(time, time + 5000, (this.axis + 1 + Math.floor(Math.random() * 2)) % 3);
        switch (this.axis) {
            case Axis.X:
                var dxf = dx * fraction;
                var x2 = x + dxf;
                ...
                vertices.push(x2, y, z, x2, y + dy, z, x2, y, z + dz, x2, y, z + dz, x2, y + dy, z, x2, y + dy, z + dz);
                break;
            case Axis.Y:
                var dyf = dy * fraction;
                var y2 = y + dyf;
                ...
                vertices.push(x, y, z, x + dx, y, z, x, y, z + dz, x, y, z + dz, x + dx, y, z, x + dx, y, z + dz);
                break;
            case Axis.Z:
                var dzf = dz * fraction;
                var z2 = z + dzf;
                ...
                vertices.push(x, y, z, x + dx, y, z, x, y + dy, z, x, y + dy, z, x + dx, y, z, x + dx, y + dy, z);
                break;
        }
        var r = this.color[0];
        var g = this.color[1];
        var b = this.color[2];
        colors.push(r, g, b, alpha, r, g, b, alpha, r, g, b, alpha, r, g, b, alpha, r, g, b, alpha, r, g, b, alpha);
...
while (true) {
    ...
    gl_matrix_1.mat4.lookAt(mvMatrix, gl_matrix_1.vec3.fromValues(-1, 2, 3), gl_matrix_1.vec3.fromValues(0, 0, 0), gl_matrix_1.vec3.fromValues(0, 1, 0));
    gl_matrix_1.mat4.rotateX(mvMatrix, mvMatrix, timeNow / 5000);
    gl_matrix_1.mat4.rotateY(mvMatrix, mvMatrix, timeNow / 6123.34);
    gl_matrix_1.mat4.rotateZ(mvMatrix, mvMatrix, timeNow / 7892.12);
